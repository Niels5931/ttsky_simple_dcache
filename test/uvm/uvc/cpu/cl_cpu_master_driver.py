...
class cl_cpu_master_driver(cl_cpu_base_driver):
    ...
    async def drive_item(self, req: cl_cpu_seq_item, rsp: cl_cpu_seq_item) -> None:
        self.logger.debug(f"drive_item: Starting {req}")
        ...
        self.logger.debug(f"drive_item: Drove request addr=0x{req.addr:02x} op={req.op.name}")
        ...
        while True:
            ...
            if (uo_val >> 4) & 1:
                self.logger.debug("drive_item: Saw req_ready=1, handshake complete")
                break
        ...
        self.drive_reset()
        self.logger.debug(f"drive_item: Complete, rsp={rsp}")
